...
(async () => {
  try {
    ...
    if (!Array.isArray(bus) && bus.data) bus = bus.data; // Handle pagination wrapping if any
    ...
    if (bus.length === 0) {
      ...
      bus = buRes.data?.data || [];
      if (!Array.isArray(bus) && bus.data) bus = bus.data;
    } else {
      console.log("Found existing Business Unit");
    }
    const bu = bus[0];
    buId = bu.id;
    ...
    if (!Array.isArray(branches) && branches.data) branches = branches.data;
    ...
    if (!Array.isArray(warehouses) && warehouses.data) warehouses = warehouses.data;
    ...
    if (warehouses.length === 0) {
      ...
      console.log("Found existing Warehouse");
    }
    ...
    // STEP 4: POS Terminal
    console.log("\nFetching POS Terminals...");
    let termRes = await api.get('/pos/terminals');
    let terminals = termRes.data?.data || [];
    if (!Array.isArray(terminals) && terminals.data) terminals = terminals.data;
    ...
    if (terminals.length === 0) {
      ...
      console.log("Found existing POS Terminal");
    }
    const terminal = terminals[0];
    terminalId = terminal.id;
    console.log(`> POS Terminal: ID=${terminal.id} Code=${terminal.terminal_code} Company=${terminal.company_id} Warehouse=${terminal.warehouse_id}`);

    console.log("\n--- HIERARCHY VALIDATION SUCCESS ---");
  } catch (e) {
