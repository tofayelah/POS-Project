...
(async () => {
  try {
    ...
    if (!Array.isArray(bus)) {
        if (bus.data) bus = bus.data; // Sometimes Laravel wraps pagination
    }
    if (bus.length === 0) {
      console.log("Business Unit array is empty. This is strange because we just created one.");
    } else {
        const bu = bus[0];
        buId = bu.id;
    ...
    if (!buId) {
        console.log("BU Res:", JSON.stringify(buRes.data));
        return;
    }
    ...
    if (!Array.isArray(branches)) if (branches.data) branches = branches.data;
    ...
    if (!Array.isArray(warehouses)) if (warehouses.data) warehouses = warehouses.data;
    ...
    if (!Array.isArray(terminals)) if (terminals.data) terminals = terminals.data;
